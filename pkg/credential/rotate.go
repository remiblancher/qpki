...
func RotateCredentialVersioned(caInstance *ca.CA, req CredentialRotateRequest) (*CredentialRotateResult, error) {
	...
	cred, err := req.CredentialStore.Load(ctx, req.CredentialID)
	if err != nil {
		return nil, fmt.Errorf("failed to load credential: %w", err)
	}

	// Use provided profiles or fail if none
	...
	// Set base path
	credDir := CredentialPath(req.CredentialStore.BasePath(), req.CredentialID)
	...
	for _, prof := range profiles {
		...
		if prof.IsCatalyst() {
			cert, signers, _, issueErr = issueCatalystCertFromProfile(caInstance, enrollReq, prof, notBefore, notAfter, req.CredentialID, keyIndex)
			keyIndex += 2
		} else if prof.IsComposite() {
			cert, signers, _, issueErr = issueCompositeCertFromProfile(caInstance, enrollReq, prof, notBefore, notAfter, req.CredentialID, keyIndex)
			keyIndex += 2
		} else {
			var signer pkicrypto.Signer
			cert, signer, _, issueErr = issueSimpleCertFromProfile(caInstance, enrollReq, prof, notBefore, notAfter, req.CredentialID, keyIndex)
			if signer != nil {
		...
		if err := SaveVersion(req.CredentialStore.BasePath(), req.CredentialID, newVersionID, algoFamily,
...
func RotateCredential(caInstance *ca.CA, ctx context.Context, credentialID string, credentialStore Store, profileStore profile.Store, passphrase []byte, keyMode KeyRotationMode, newProfiles []string) (*EnrollmentResult, error) {
	// Load existing credential
	existingCredential, err := credentialStore.Load(ctx, credentialID)
	if err != nil {
		return nil, fmt.Errorf("failed to load credential: %w", err)
	}
	...
	if keyMode == KeyRotateKeep {
		...
		existingSigners, err := credentialStore.LoadKeys(ctx, credentialID, passphrase)
		if err != nil {
		...
		result, err = rotateWithExistingKeys(caInstance, req, profiles, existingSigners)
		if err != nil {
		...
		if len(profiles) == 1 {
			result, err = EnrollWithProfile(caInstance, req, profiles[0])
		} else {
	...
	// Save new credential
	if err := credentialStore.Save(ctx, result.Credential, result.Certificates, result.Signers, passphrase); err != nil {
		return nil, fmt.Errorf("failed to save credential: %w", err)
	}
...
func rotateWithExistingKeys(caInstance *ca.CA, req EnrollmentRequest, profiles []*profile.Profile, existingSigners []pkicrypto.Signer) (*EnrollmentResult, error) {
	if len(profiles) == 0 {
	...
	for _, prof := range profiles {
		...
		if prof.IsCatalyst() {
			cert, signers, err = issueCatalystCertWithExistingKeys(caInstance, req, prof, notBefore, notAfter, signersByAlg, usedSignerIndex)
		} else if prof.IsComposite() {
			cert, signers, err = issueCompositeCertWithExistingKeys(caInstance, req, prof, notBefore, notAfter, signersByAlg, usedSignerIndex)
		} else {
			var signer pkicrypto.Signer
			cert, signer, err = issueSimpleCertWithExistingSigner(caInstance, req, prof, notBefore, notAfter, signersByAlg, usedSignerIndex)
			if signer != nil {
				signers = []pkicrypto.Signer{signer}
			}
		}
...
func issueSimpleCertWithExistingSigner(caInstance *ca.CA, req EnrollmentRequest, prof *profile.Profile, notBefore, notAfter time.Time, signersByAlg map[pkicrypto.AlgorithmID][]pkicrypto.Signer, usedIndex map[pkicrypto.AlgorithmID]int) (*x509.Certificate, pkicrypto.Signer, error) {
...
func issueCatalystCertWithExistingKeys(caInstance *ca.CA, req EnrollmentRequest, prof *profile.Profile, notBefore, notAfter time.Time, signersByAlg map[pkicrypto.AlgorithmID][]pkicrypto.Signer, usedIndex map[pkicrypto.AlgorithmID]int) (*x509.Certificate, []pkicrypto.Signer, error) {
...
func issueCompositeCertWithExistingKeys(caInstance *ca.CA, req EnrollmentRequest, prof *profile.Profile, notBefore, notAfter time.Time, signersByAlg map[pkicrypto.AlgorithmID][]pkicrypto.Signer, usedIndex map[pkicrypto.AlgorithmID]int) (*x509.Certificate, []pkicrypto.Signer, error) {
...
func RevokeCredential(caInstance *ca.CA, ctx context.Context, credentialID string, reason ca.RevocationReason, credentialStore Store) error {
	// Load credential
	cred, err := credentialStore.Load(ctx, credentialID)
	if err != nil {
		return fmt.Errorf("failed to load credential: %w", err)
	}

	// Load all certificates from active version and revoke them
	certs, err := credentialStore.LoadCertificates(ctx, credentialID)
	if err != nil {
	...
	if err := credentialStore.UpdateStatus(ctx, credentialID, StatusRevoked, reason.String()); err != nil {
