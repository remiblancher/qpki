...
func runVerify(cmd *cobra.Command, args []string) error {
	...
	// Initialize result
	result := &cli.VerifyResult{IsValid: true}

	// Check signature
	chainErr := cli.VerifyCertificateSignature(cert, caCert, intermediates)

	// Check validity period
	validPeriod, statusMsg, expiredInfo := cli.CheckValidityPeriod(cert)
	if !validPeriod {
	...
	// Check revocation
	revoked, revocationInfo, err := cli.CheckRevocationStatus(cert, caCert, verifyCRLFile, verifyOCSPURL)
	if err != nil {
		return err
	}
	if revoked {
